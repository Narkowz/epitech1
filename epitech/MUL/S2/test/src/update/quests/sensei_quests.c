...
static void final_quest(npc_t *sensei, rpg_t *rpg, sfEvent *event)
{
    switch (rpg->player.quest) {
        case 44:
            if (rpg->lang == ENGLISH)
                tuto_quest_eng(sensei, rpg, event, "ggwp");
            else
                tuto_quest(sensei, rpg, event, "ggwp");
            break;
        default:
            break;
    }
