...
void load_progress(sfRectangleShape *loading_bar,
    sfRectangleShape *background, rpg_t *rpg, hud_t **hud)
{
    ...
    for (int i = 0; i < 100; i += 15) {
        ...
        DRAW_SPRITE(rpg->window, (*hud)->menu->back_sprite, NULL);
        DRAW_SPRITE(rpg->window, (*hud)->menu->name_sprite, NULL);
    ...
    stats_t **stats)
{
    ...
    if (init_rpg(rpg, head, hud, stats) == 84)
        return 84;
    load_progress(loading_bar, background, rpg, hud);
