...
button_t *init_button(button_t *button, window_t *window)
{
    ...
    sfRectangleShape_setPosition(button->rect, (sfVector2f) {sfRenderWindow_getSize(window->window).x / 2, sfRenderWindow_getSize(window->window).y / 2});
    sfRectangleShape_setFillColor(button->rect, sfMagenta);
    return button;
}
