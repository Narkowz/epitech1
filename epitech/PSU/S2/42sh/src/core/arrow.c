...
{
    if (!shell->actual_save)
        shell->actual_save = shell->save;
    if (shell->actual_save && shell->actual_save->prec) {
        shell->actual_save = shell->actual_save->prec;
    } else {
        return;
    }
    shell->save->input = my_strdup(shell->actual_save->raw_input);
...
{
    if (shell->actual_save && shell->actual_save->next) {
        shell->actual_save = shell->actual_save->next;
    } else {
        return;
    }
    if (!shell->actual_save->raw_input)
