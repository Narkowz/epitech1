...
*/
    ...
    #define LAST_RREDIR (last_sep == 4 || last_sep == 2)
    #define LAST_LREDIR (last_sep == 5 || last_sep == 3)

    #define EXIST_EMPTY (cpy->cmd && my_strlen(cpy->cmd) == 0)
    #define EXIST_NEMPTY (cpy->cmd && my_strlen(cpy->cmd) != 0)
    #define EXIST_ERROR (NOT_EXIST || EXIST_EMPTY)
    ...
    #define IS_PIPE (cpy->command->sep == 12)
    #define IS_INVDIR (cpy->command->sep == 5 || cpy->command->sep == 3)
    #define IS_AND (cpy->command->sep == 11)
    #define IS_OR (cpy->command->sep == 1)
