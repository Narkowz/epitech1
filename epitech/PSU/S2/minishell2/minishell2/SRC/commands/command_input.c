...
static int han_command(env_t *my_env, char **args, int *iterate, list_t *multi)
{
    int redirect;
    int pipe = handle_pipe(multi, args, my_env, iterate);

    if (pipe != 0)
        return 1;
    redirect = handle_redirection(multi, args, my_env, iterate);
    if (redirect == 84)
        exit(1);
    if (redirect == 1)
        return 1;
...
int loop_for_args(env_t *my_env, char ***args, int *value, list_t *multi)
{
    ...
    while (multi != NULL) {
        ...
        if (han_command(my_env, *args, &iterate, multi) == 0) {
            multi->command = strtok(multi->command, " \t\n");
            temp_if(multi, args, my_env, value);
        }
        iterate++;
        while (i < iterate && multi != NULL) {
            multi = multi->next;
            i++;
        }
    }
