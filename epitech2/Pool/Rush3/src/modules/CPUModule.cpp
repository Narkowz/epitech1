...
namespace Krell {
    ...
    #ifdef __APPLE__ // FOUND CPU MODEL APPLE
    ...
    std::string CPUModule::getCPUFrequency() const
    {
        uint64_t frequency;
        size_t size = sizeof(frequency);
        if (sysctlbyname("hw.cpufrequency", &frequency, &size, NULL, 0) == 0)
            return std::to_string(frequency / 1'000'000) + " MHz";
        return "Unknown Frequency";
    }

    #elif __linux__ // FOUND CPU MODEL LINUX
